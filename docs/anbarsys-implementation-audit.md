...
| Zone inventory and FIFO override | Complete | Movement history, FIFO rule and audited override |
...
| Session auto-save and recovery | Partial | Draft persistence/resume exists; timeout, administrative termination, handover and complete lifecycle remain |
...
| Fine-grained operational RBAC | Missing | Generic OPERATOR role does not implement station-specific roles, skills and task-based authorization |
| Complete audit record | Partial | Session/device exist; user, request ID, IP, result and before/after state remain |
