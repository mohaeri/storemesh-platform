...
test('sorting entry and exit are separate persistent operator flows with weighing only in the scale console',()=>{assert.match(assembled,/ورود به سورتینگ/);assert.match(assembled,/خروج از سورتینگ/);assert.match(assembled,/sortingSessions: any\[\]/);assert.match(assembled,/status: "IN_PROGRESS"/);assert.match(assembled,/currentState: "IN_SORTING"/);assert.match(assembled,/nextAction: "ثبت خروجی سورتینگ"/);assert.match(assembled,/function openExit\(\)/);assert.match(assembled,/activeSortingSession\.inputCodes/);assert.match(assembled,/ثبت وزن جدید/);assert.match(assembled,/دریافت وزن خروجی/);assert.doesNotMatch(assembled,/دریافت وزن از ترازو \(اختیاری\)|وضعیت لودسل/)});
...
test('washing has independent persistent entry and exit operator views with a shared top scale console',()=>{assert.match(assembled,/function WashingScaleConsole/);assert.match(assembled,/mode = initialMode/);assert.match(assembled,/initialMode="ENTRY"/);assert.match(assembled,/initialMode="EXIT"/);assert.match(assembled,/ورودی‌ها ذخیره شده‌اند و پس از خاموش و روشن شدن سیستم/);assert.match(assembled,/وزن هر خروجی فقط از باسکول دریافت می‌شود/);assert.doesNotMatch(assembled,/وزن خالص خروجی \(kg\)/)});
