...
test('washing entry reuses optional weight states and shows the sorting destination',()=>{assert.match(assembled,/captureWashingEntryWeight/);assert.match(assembled,/pwSelectSortingEntryWeightState\(entryWeightStates,weighingCode,item\.containerCode,entryWeights\)/);assert.match(assembled,/weightState==="PENDING"\?"در انتظار ثبت وزن":"وزن قبلی انتخاب شد"/);assert.match(assembled,/finalDestination=item\.operationalDestination\|\|item\.destination/);assert.match(assembled,/>مقصد نهایی<\/small>/);assert.doesNotMatch(assembled,/>مقصد بعدی<\/small><b>\{PW_ZONES\[item\.nextZone/)});
