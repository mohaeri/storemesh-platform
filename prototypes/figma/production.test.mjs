...
test('production workbench follows the approved operation map without physical merge navigation',()=>{assert.match(assembled,/عملیات نظارتی/);assert.match(assembled,/\["sorting-entry",\s*"ورود به سورتینگ"/);assert.match(assembled,/\["sorting-exit",\s*"خروج از سورتینگ"/);assert.match(assembled,/initialStep="input"/);assert.match(assembled,/initialStep="output"/);assert.match(assembled,/ثبت ورود یا خروج شست‌وشو/);assert.match(assembled,/ثبت خروج از فریز و بسته‌بندی/);assert.match(assembled,/ثبت ورود به اسلایس/);assert.match(assembled,/ثبت خروج از خشک‌کن و بسته‌بندی/);assert.match(assembled,/ثبت ورود به فریزدرای/);assert.match(assembled,/ثبت خروج از فریزدرای و بسته‌بندی/);assert.doesNotMatch(assembled,/\["merge", "ادغام فیزیکی"\]/)});
test('washing has independent persistent entry and exit operator views',()=>{assert.match(assembled,/mode, setMode.*"ENTRY"/s);assert.match(assembled,/mode === "ENTRY"/);assert.match(assembled,/mode === "EXIT"/);assert.match(assembled,/ورودی‌ها ذخیره شده‌اند و پس از خاموش و روشن شدن سیستم/);assert.match(assembled,/ثبت سبدهای تازه، وزن و مسیر بعدی/)});
