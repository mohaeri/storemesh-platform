...
test('sorting discovers cold-room baskets routed to sorting and suggests the next scan',()=>{assert.match(assembled,/const eligibleSources = batch\.baskets\.filter/);assert.match(assembled,/b\.destination === "SORTING"/);assert.match(assembled,/b\.currentLocation \|\| b\.zone/);assert.match(assembled,/سبدهای شناسایی‌شده برای سورت/);assert.match(assembled,/suggestedCode=\{eligibleSources\[0\]\?\.code\|\|""\}/)});
