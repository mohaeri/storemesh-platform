...
test('production workbench follows the approved operation map without physical merge navigation',()=>{assert.match(assembled,/عملیات نظارتی/);assert.match(assembled,/ثبت ورود یا خروج سورتینگ/);assert.match(assembled,/ثبت ورود یا خروج شست‌وشو/);assert.match(assembled,/ثبت خروج از فریز و بسته‌بندی/);assert.match(assembled,/ثبت ورود به اسلایس/);assert.match(assembled,/ثبت خروج از خشک‌کن و بسته‌بندی/);assert.match(assembled,/ثبت ورود به فریزدرای/);assert.match(assembled,/ثبت خروج از فریزدرای و بسته‌بندی/);assert.doesNotMatch(assembled,/\["merge", "ادغام فیزیکی"\]/)});
