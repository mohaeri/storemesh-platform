...
function ReceivingScreen({ navigate }: { navigate: (s: WebScreen) => void }) {
  ...
  const resetReceiving=()=>{setStage("setup");setBaskets([]);setDestination("");setMoveMode("batch");setTransferBatch(null);setDispatchStarted(false);setMovedCodes([]);setTransferError("")};
  ...
  return <div className="flex-1 bg-[#f4f7f5] p-5 overflow-auto" dir="rtl">
    <div className="flex items-start justify-between mb-4"><div><p className="text-[#176b50] text-[11px] font-bold">دریافت · {stage==="setup"?"محموله جدید":reference||"محموله جاری"}</p><h2 className="font-['Vazirmatn:Bold',sans-serif] font-bold text-[#18302a] text-[22px]">{stage==="setup"?"تعریف محموله ورودی":stage==="review"?"بازبینی محموله":stage==="dispatch"?"انتخاب مقصد و تحویل":"ثبت و توزین ظروف"}</h2><p className="font-['Vazirmatn:Regular',sans-serif] text-[#718079] text-[13px]">{stage==="setup"?"اطلاعات بار را ثبت کنید؛ سپس ظروف را یکی‌یکی اسکن و توزین کنید.":supplier+" · پیشرفت "+baskets.length+" از "+expected+" ظرف"}</p></div><Badge text={stage==="setup"?"مرحله ۱ از ۴":stage==="capture"?"مرحله ۲ از ۴":stage==="review"?"مرحله ۳ از ۴":"مرحله ۴ از ۴"} color="#176b50" bg="#e1f2eb" /></div>
    ...
    {stage==="dispatch"&&<Card className="mt-4 p-5"><div className="flex items-start justify-between"><div><h3 className="font-bold text-[#18302a] text-[16px]">۴. مقصد و تحویل محموله</h3><p className="text-[11px] text-[#718079] mt-1">انتقال عادی همین‌جا تکمیل می‌شود؛ برای ثبت آن لازم نیست به صفحه موجودی بروید.</p></div><Badge text={`${movedCodes.length} از ${baskets.length} ظرف تحویل‌شده`} color="#176b50" bg="#e1f2eb"/></div>{transferError&&<p role="alert" className="bg-[#fbe7e7] text-[#a43838] p-3 rounded-lg mt-4 text-[11px]">{transferError}</p>}{!dispatchStarted?<><label className="block text-[11px] font-bold mt-5">مقصد مستقیم محموله<select value={destination} onChange={e=>setDestination(e.target.value)} className={selectClass}><option value="">انتخاب مقصد…</option>{Object.entries(destinationNames).map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label><div className="grid grid-cols-2 gap-3 mt-4"><button onClick={()=>setMoveMode("batch")} className={(moveMode==="batch"?"border-[#176b50] bg-[#eaf6f0]":"border-[#d8e4df]")+" border-2 rounded-xl p-4 text-right"}><b className="text-[12px]">انتقال کل بچ یکجا</b><p className="text-[10px] text-[#718079] mt-1">همه {baskets.length} ظرف با یک تأیید در مقصد ثبت می‌شوند.</p></button><button onClick={()=>setMoveMode("scan")} className={(moveMode==="scan"?"border-[#176b50] bg-[#eaf6f0]":"border-[#d8e4df]")+" border-2 rounded-xl p-4 text-right"}><b className="text-[12px]">اسکن تک‌تک در همین صفحه</b><p className="text-[10px] text-[#718079] mt-1">کد سبد بعدی خودکار داخل شبیه‌ساز قرار می‌گیرد.</p></button></div><div className="flex justify-end gap-2 mt-5"><button onClick={()=>setStage("review")} className="border rounded-lg px-4 h-11 text-[11px]">بازگشت</button><button disabled={!destination} onClick={startDispatch} className="bg-[#176b50] disabled:opacity-40 text-white rounded-lg px-6 h-11 text-[12px] font-bold">{moveMode==="batch"?"انتقال و تکمیل کل بچ":"شروع اسکن در مقصد"}</button></div></>:<div className="mt-5 grid grid-cols-[1fr_280px] gap-4"><div><button onClick={()=>setTransferScanOpen(true)} className="w-full h-20 border-2 border-dashed border-[#176b50] rounded-xl text-[#176b50] font-bold">⌗ اسکن سبد بعدی در {destinationNames[destination]}<small className="block mt-1 font-mono">{baskets.find(item=>!movedCodes.includes(item.code))?.code}</small></button><div className="mt-3 max-h-48 overflow-auto">{movedCodes.slice().reverse().map(code=><div key={code} className="flex justify-between border-b py-2 text-[11px]"><span className="text-[#176b50]">تحویل مقصد ✓</span><b className="font-mono">{code}</b></div>)}</div></div><div className="bg-[#edf8f3] rounded-xl p-4"><b className="text-[12px]">مقصد: {destinationNames[destination]}</b><p className="text-[10px] text-[#718079] mt-2">هر اسکن همان لحظه موقعیت، وضعیت و سابقه ظرف را ثبت می‌کند. با آخرین ظرف، دریافت خودکار بسته می‌شود.</p><div className="mt-4 text-center text-[26px] font-bold text-[#176b50]">{movedCodes.length}/{baskets.length}</div></div></div>}</Card>}
